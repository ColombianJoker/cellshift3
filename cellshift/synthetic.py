        ...
        if n_uniques>max_uniques:
            try:
                ...
                for row_id in tqdm(range(all_count),
                                   disable=not (verbose and (n_uniques>1000))):
                    fake_name=fake.name()
                    update_sql = f"UPDATE \"{self._tablename}\" SET \"{new_column_name}\"='{fake_name}' WHERE rowid=={row_id};"
                    # if verbose:
                    #     print(f"{update_sql=}", file=sys.stderr)
                    self.cx.execute(update_sql)
            except Exception as e:
...
def add_syn_first_name_column(self, 
                        base_column: str, 
                        new_column_name: Optional[str] = None, 
                        max_uniques: Optional[int] = 1000,
                        verbose: bool = False) -> CS:
    ...
    if (new_column_name is None):
      if (isinstance(base_column, str)):
          ...
          raise ValueError("add_syn_first_name_column: no new_column_name given and base_column is not valid.")
    fake=Faker(self.faker_locale)
    Faker.seed(self.faker_seed)
    if isinstance(base_column,str):
        # Get unique names in base_column
        n_uniques = self.cx.sql(f'SELECT COUNT(DISTINCT "{base_column}") FROM "{self._tablename}";').fetchall()[0][0]
        if verbose:
          print(f"add_syn_first_name_column: processing '{self._tablename}', with {n_uniques} values.", file=sys.stderr)
        if n_uniques>max_uniques:
            try:
                # Generate without equivalences
                add_column_sql = f'ALTER TABLE "{self._tablename}" ADD COLUMN "{new_column_name}" VARCHAR;'
            ...
            try:
                ...
                self.cx.execute(update_syn_names)
                self.data = self.cx.table(self._tablename)
                self.first_name_equivalences = self.cx.table(new_table_name)
            except Exception as e:
                printf(f"{e=}", file=sys.stderr)
                raise Exception
    return self

def syn_first_name_column(self,
                    base_column: str,
                    max_uniques: Optional[int] = 1000,
                    verbose: bool = False) -> 'CS':
        """
        ...
        Returns:
            self: The CS object with the 'base_column' replaced by synthetic person first names.
        """
        new_syn_column_name = f"syn_{base_column}"

        if verbose:
            print(f"syn_first_name_column: Replacing '{base_column}' with synthetic first names.", file=sys.stderr)
        ...
        try:
            ...
            if verbose:
                print(f"syn_first_name_column: Successfully added '{new_syn_column_name}'.", file=sys.stderr)
        except Exception as e:
            print(f"syn_first_name_column: Error adding synthetic column: {e}", file=sys.stderr)
            raise
        ...
        try:
            self.replace_column(base_column, new_syn_column_name)
            if verbose:
                print(f"syn_first_name_column: Successfully replaced '{base_column}' with '{new_syn_column_name}'.", file=sys.stderr)
        except Exception as e:
            print(f"syn_first_name_column: Error replacing column: {e}", file=sys.stderr)
            raise
        ...
        try:
            self.drop_column(new_syn_column_name)
            if verbose:
                print(f"syn_first_name_column: Successfully dropped temporary column '{new_syn_column_name}'.", file=sys.stderr)
        except Exception as e:
            print(f"syn_first_name_column: Error dropping temporary column '{new_syn_column_name}': {e}", file=sys.stderr)
            raise

        return self

def add_syn_last_name_column(self, 
                        base_column: str, 
                        new_column_name: Optional[str] = None, 
                        max_uniques: Optional[int] = 1000,
                        verbose: bool = False) -> CS:
    ...
    if isinstance(base_column,str):
        ...
        if n_uniques>max_uniques:
            ...
            try:
                ...
                self.cx.execute(update_syn_names)
                self.data = self.cx.table(self._tablename)
                self.last_name_equivalences = self.cx.table(new_table_name)
            except Exception as e:
                printf(f"{e=}", file=sys.stderr)
                raise Exception
    return self

def syn_last_name_column(self,
                    base_column: str,
                    max_uniques: Optional[int] = 1000,
                    verbose: bool = False) -> 'CS':
        """
        ...
        Returns:
            self: The CS object with the 'base_column' replaced by synthetic person last names.
        """
        new_syn_column_name = f"syn_{base_column}"
...
def add_syn_class_column(self,
                         base_column: str, 
                         new_column_name: Optional[str] = None, 
                         max_uniques: Optional[int] = 1000,
                         verbose: bool = False) -> CS:
    """
    Adds a column with synthetic equal-length LETTER classes, calculating based on values of base_column.
    ...
    if (new_column_name is None):
      if (isinstance(base_column, str)):
          ...
          raise ValueError("add_syn_class_column: no new_column_name given and base_column is not valid.")
    fake=Faker(self.faker_locale)
    Faker.seed(self.faker_seed)
    if isinstance(base_column,str):
        # Get unique names in base_column
        n_uniques = self.cx.sql(f'SELECT COUNT(DISTINCT "{base_column}") FROM "{self._tablename}";').fetchall()[0][0]
        if verbose:
          print(f"add_syn_class_column: processing '{self._tablename}', with {n_uniques} values.", file=sys.stderr)
        n_letters=letters_for(len(str(n_uniques)))
        if verbose:
            print(f"add_syn_class_column: will use {n_letters} for classes.")
        if n_uniques>max_uniques:
            try:
                # Generate without equivalences
                add_column_sql = f'ALTER TABLE "{self._tablename}" ADD COLUMN "{new_column_name}" VARCHAR;'
                ...
                for row_id in tqdm(range(all_count),
                    ...
                    update_sql = f"UPDATE \"{self._tablename}\" SET \"{new_column_name}\"='{fake_code}' WHERE rowid=={row_id};"
                    # if verbose:
                    #     print(f"{update_sql=}", file=sys.stderr)
                    self.cx.execute(update_sql)
            except Exception as e:
            ...
            try:
                ...
                for row_id in tqdm(range(n_uniques),
                                   disable=not (verbose and (n_uniques>1000))):
                    fake_code=random_code(n_letters)
                    update_sql = f"UPDATE \"{new_table_name}\" SET \"{new_column_name}\"='{fake_code}' WHERE rowid=={row_id};"
                    if verbose:
                        print(f"{update_sql=}", file=sys.stderr)
                    self.cx.execute(update_sql)
                update_syn_classes = f"""
                    UPDATE "{self._tablename}"
                    SET "{new_column_name}"="{new_table_name}"."{new_column_name}"
                    FROM "{new_table_name}" 
                    WHERE "{self._tablename}"."{base_column}"=="{new_table_name}"."{base_column}"
                ...
                if verbose:
                    print(f"{add_column_sql=}", file=sys.stderr)
                    print(f"{update_syn_classes=}", file=sys.stderr)
                self.cx.sql(add_column_sql)
                self.data = self.cx.table(self._tablename)
                self.cx.execute(update_syn_classes)
                self.data = self.cx.table(self._tablename)
                self.class_equivalences = self.cx.table(new_table_name)
            except Exception as e:
